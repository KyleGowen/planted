-- Registration / reanalysis v14: add the placementNotesSummary output field. This is a
-- short paraphrase of the owner's placement notes (the {{location}} prompt variable) used
-- to render a third line under the Indoor/Outdoor label in the plant detail header. It is
-- distinct from placementGuidance (advice) and placementGeneralGuidance (educational).
...
PLACEMENT NOTES SUMMARY (placementNotesSummary field):
This field is a short paraphrase of what the OWNER wrote in their placement notes ({{location}} in the user prompt), rendered as a caption under the Indoor/Outdoor label in the UI. It is NOT advice and NOT species context.
- Exactly one short sentence or phrase, roughly 120 characters or fewer; no bullet points or multi-line text. Prefer a neutral noun-phrase style, e.g. "Living room east window with morning sun" or "South-facing balcony, partial afternoon shade".
- Paraphrase the owner's own words; correct obvious typos, drop filler, and compress multi-line notes to the single most useful fact about where the plant lives. Do not paste the raw notes verbatim if they are longer than one sentence.
- Stay factual about what the owner said. Do not add a species name, care advice, recommendations, climate/region the owner did not mention, or evaluative words like "great" / "too dark" / "perfect". Do not invent details the owner did not state (e.g. window direction, floor level, light duration).
- If the owner's placement notes are missing, blank, or contain no usable placement info, return an empty string. Do not fabricate a location, and do not fall back to the geographic_location or owner_physical_address fields — those describe region, not placement within the home.$v14sys$,
    'text',
...
(
    'plant_registration_analysis_v1', 14, 'user',
    $v14usr$Please analyze this plant image.
...
{{#if location}}Owner placement notes (use as context for placementGuidance and placementGeneralGuidance — synthesize actionable guidance; do not copy or quote this text verbatim there — AND as the source for placementNotesSummary, where you paraphrase this text into one short caption): {{location}}{{/if}}
...
For placementNotesSummary: if the owner supplied placement notes above, paraphrase them into one short caption (<=120 chars, no advice, no invented detail). If no placement notes are present, return an empty string for placementNotesSummary.
...
Return a complete JSON analysis matching the required schema.$v14usr$,
    'handlebars',
    '{"required": [], "optional": ["plant_name", "goals_text", "location", "geographic_location", "owner_physical_address", "prior_care_context", "care_history", "history_notes"]}',
    ARRAY['registration', 'analysis']
)
