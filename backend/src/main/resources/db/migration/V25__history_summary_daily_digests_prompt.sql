-- History summary: per-calendar-day narrative digests (structured JSON) instead of line-per-item timeline.

UPDATE llm_prompts
SET content = $hist_summ_sys_v25$You are a careful assistant producing short daily digests for a single houseplant's documented history for the owner's History section.
...
- Do not repeat the same calendar day twice in daily_digests.$hist_summ_sys_v25$,
    updated_at = NOW()
...
{{/if}}Below is the owner's history. The first block groups events by local calendar day with explicit care counts per day; the detailed chronological section repeats the same facts. Build daily_digests from these sources only.
{{#if image_count}}Attached images: {{image_count}}.
{{/if}}

{{timeline_text}}$hist_summ_usr_v25$,
    updated_at = NOW()
WHERE prompt_key = 'plant_history_summary_v2'
  AND version = 1
  AND role = 'user';
