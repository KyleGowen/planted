-- Registration / reanalysis v12: replace the single-sentence health instruction with a dedicated
-- HEALTH ASSESSMENT block that forces a realistic evaluation. The model must not default to
-- "healthy" on plants that show visible signs of stress, damage, pests, or disease, and must
-- describe specific visible issues (what and where) with plausible causes rather than a vague
-- catch-all sentence. If the image is insufficient, say so instead of declaring the plant healthy.
...
- For each notable finding, offer 1–3 plausible causes (for example: overwatering or underwatering, low or excessive light, low humidity, nutrient deficiency, pests, root or pot issues, recent environment change, sunburn, mechanical damage). Do NOT lock to a single cause, and in particular do not assume watering is the only explanation.
...
- Tone: honest and specific, but not alarmist — no medical-style panic. Beginner-friendly phrasing.
- You may close with at most one or two concrete next checks the owner can do themselves (e.g. check soil moisture a couple inches down, inspect the undersides of leaves for pests, move away from a hot window). Keep this short; do not turn healthDiagnosis into a full care plan.
...
(
    'plant_registration_analysis_v1', 12, 'user',
    $v12usr$Please analyze this plant image.
...
{{history_notes}}{{/if}}
Evaluate plant health realistically; do not default to "healthy" if visible signs of stress, damage, pests, or disease are present. Describe specific visible issues (what and where) when you see them, and offer plausible causes without assuming a single one.
...
Return a complete JSON analysis matching the required schema.$v12usr$,
    'handlebars',
    '{"required": [], "optional": ["plant_name", "goals_text", "location", "geographic_location", "owner_physical_address", "prior_care_context", "care_history", "history_notes"]}',
    ARRAY['registration', 'analysis']
)
