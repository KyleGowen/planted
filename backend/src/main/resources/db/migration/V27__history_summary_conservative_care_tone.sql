-- History summary digests: conservative tone; flag implausible/heavy same-day care; no sycophantic praise.

UPDATE llm_prompts
SET content = $hist_summ_sys_v27$You are a careful assistant producing short daily digests for a single houseplant's documented history for the owner's History section.
...
- Prefer neutral clinical phrasing: state what was logged, then one short caution when counts alone warrant it.$hist_summ_sys_v27$,
    updated_at = NOW()
...
{{timeline_text}}$hist_summ_usr_v27$,
    updated_at = NOW()
WHERE prompt_key = 'plant_history_summary_v2'
  AND version = 1
  AND role = 'user';
