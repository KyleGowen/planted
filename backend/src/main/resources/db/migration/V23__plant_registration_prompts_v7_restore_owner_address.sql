-- v7: Restore owner's physical address in registration prompts (removed accidentally when v6 pruning text landed in V22).
...
(
    'plant_registration_analysis_v1', 7, 'user',
    $v7usr$Please analyze this plant image.
...
For pruningActionSummary specifically: always include when to prune this individual plant (timing or clear trigger) and how much to remove (conservative degree), using the photo plus goals, location, geography, owner's physical address when provided, and care history when present.
Return a complete JSON analysis matching the required schema.$v7usr$,
    'handlebars',
    '{"required": [], "optional": ["plant_name", "goals_text", "location", "geographic_location", "owner_physical_address", "prior_care_context", "care_history", "history_notes"]}',
    ARRAY['registration', 'analysis']
)
