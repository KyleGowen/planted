-- v8: Treat owner placement free text as prompt context only; discourage verbatim echo in placement JSON fields.
...
(
    'plant_registration_analysis_v1', 8, 'user',
    $v8usr$Please analyze this plant image.
...
Return a complete JSON analysis matching the required schema.$v8usr$,
    'handlebars',
    '{"required": [], "optional": ["plant_name", "goals_text", "location", "geographic_location", "owner_physical_address", "prior_care_context", "care_history", "history_notes"]}',
    ARRAY['registration', 'analysis']
)
