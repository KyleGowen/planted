-- Use botanical family (not order) in the taxon heading: Family Genus Species Variety.

ALTER TABLE plant_analyses
    RENAME COLUMN taxonomic_order TO taxonomic_family;
...
(
    'plant_registration_analysis_v1', 10, 'user',
    $v10usr$Please analyze this plant image.
...
Fill taxonomicFamily, genus, species (epithet only), and variety per the taxonomy rules in the system instructions.
Return a complete JSON analysis matching the required schema.$v10usr$,
    'handlebars',
    '{"required": [], "optional": ["plant_name", "goals_text", "location", "geographic_location", "owner_physical_address", "prior_care_context", "care_history", "history_notes"]}',
    ARRAY['registration', 'analysis']
)
